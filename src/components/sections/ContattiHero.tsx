...
export function ContattiHero() {
  ...
  return (
    <section className="relative h-screen min-h-[560px] overflow-hidden">
      <Image
        src="/images/hotel-esterno.jpg"
        alt="Villa Madau Hotel – esterno Pula"
        fill
        priority
        sizes="100vw"
        ...
        <nav className="flex gap-10 py-4" aria-label="Navigazione principale">
          {navLinks.map((link) => (
            <Link
              ...
              href={link.href}
              className={clsx(
                'text-[11px] font-medium transition-colors duration-200 tracking-[0.2em] uppercase',
                pathname === link.href ? 'text-white/50' : 'text-white hover:text-white/60'
              )}
            >
              {link.label}
            </Link>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-4">
        <p className="text-xs font-medium uppercase tracking-[0.25em] text-white/70 mb-4">
          Via Nora, 84 · Pula, Sardegna
        </p>
        <h1 className="font-serif text-5xl md:text-7xl">Contatti</h1>
      </div>
    </section>
  )
}
